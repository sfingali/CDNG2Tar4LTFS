...
class TarPackager(QObject):
    ...
    def __init__(self, directories: Iterable[Path], output_root: Path, delete_originals: bool = False) -> None:
        super().__init__()
        ...
        try:
            ...
            for directory in self.directories:
                ...
                if self.delete_originals:
                    self.progress.emit(f"Deleting original DNG files in {directory.name}...")
                    deleted_count = self._delete_original_dngs(directory)
                    self.progress.emit(f"Deleted {deleted_count} DNG files in {directory.name}.")
    ...
    @staticmethod
    def _delete_original_dngs(directory: Path) -> int:
        deleted_count = 0
...
class SummaryDialog(QDialog):
    def __init__(self, summary_text: str, parent: QWidget | None = None) -> None:
        ...
        self.setWindowTitle("Packing completed")
        self.resize(800, 500)
...
class MainWindow(QWidget):
    def __init__(self) -> None:
        ...
        self.delete_originals_checkbox = QCheckBox("Delete original CDNG .dng files after successful TAR + screenshots")
    ...
    def _on_completed(self, results: List[PackResult]) -> None:
        ...
        if self.delete_originals_checkbox.isChecked():
            screenshot_lines.append("Original CDNG .dng files were deleted after successful processing.")
        message = "\n".join(tar_lines + screenshot_lines)
        summary_dialog = SummaryDialog(message, self)
        summary_dialog.exec()
